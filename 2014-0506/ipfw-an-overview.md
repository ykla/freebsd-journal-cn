...
另一种方式是将自定义规则集文件的完整路径作为 `firewall_type` 指定，IPFW 会读取该文件并将每一行解释为 `ipfw` 命令的参数。IPFW 还支持对指定文件使用预处理器（如 m4），通过标志 `-p` 指定。这使管理员可以创建单一模板，经预处理后生成适用于不同主机的防火墙规则。
...
上述规则集创建了一条带宽为 50 兆比特每秒、最大队列为 20 个数据包的管道。随后创建两个队列，第一个的权重是第二个的 10 倍。流量随后被分入这两个队列之一。源或目的端口为 5060（SIP）的数据包，或带有标志 `IPTOS_LOWDELAY` 的数据包进入高优先级队列（40008），其余流量进入低优先级队列（40011）。这应有助于确保 VoIP 通话在网络活动高峰期不受影响。
