...
多路径客户端的操作由此开始。使用 sg3_utils 软件包中的 `sg_inq` 工具，获取 FreeBSD 报告的两个块设备 da0 和 da1 的标识信息：
