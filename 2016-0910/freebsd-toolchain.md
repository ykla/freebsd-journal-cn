...
除基本系统外，还有一个通过 FreeBSD Ports 树和二进制包管理的第三方软件生态系统。Ports 树中有超过 26,000 个第三方软件包。
...
基本系统中的 GNU ld 还缺少对 AArch64（arm64）和 RISC-V 的支持，这两种 CPU 架构是最近加入 FreeBSD 的。目前为这些架构构建 FreeBSD 需要从 Ports 树安装链接器或作为包安装，此时它会被自动使用。
...
除基本系统中的 LLDB 外，GNU GDB 调试器可通过从 Ports 树构建或安装包来使用。它已更新，包含改进的线程支持和内核调试。通过简单的 `pkg install gdb` 命令即可使用 GDB 7.11.1。
...
以 LLD 作为链接器后，GNU binutils 中仍剩两个工具：汇编器（as）和 objdump。目前没有替代 as 的候选。然而，在一级架构上构建 FreeBSD 基本系统实际上并不需要它：Clang 的集成汇编器用于构建汇编源文件。GNU as 可以直接移除，或者可以从集成汇编器构建一个与 GNU as 兼容的驱动。objdump 也不是基本系统构建所必需的，可以移除。LLVM 提供了 llvm-objdump，目前功能有限，但适时可能会成为可行的替代品。
