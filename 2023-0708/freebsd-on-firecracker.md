...
- Firecracker 通过匿名 `mmap` 分配 VM 客户机内存，但 Linux 未设置整个 VM 客户机地址空间的分页结构。结果是，第一次读取任何页面时都会发生页错误，花费大约 20,000 个 CPU 周期来处理，同时 Linux 映射一页内存。在 Firecracker 的 `mmap` 调用中添加标志 `MAP_POPULATE` 可节省 2 毫秒。（在出版时尚未提交。）
