...
请注意，一旦设置此值，所有使用 RACK 堆栈的 TCP 连接将使用 DGP，默认的 FCC 值为 30Mbps。你还可以使用 `sysctl` 更改此默认值（FCC），以更好地匹配你的网络条件。`sysctl` 变量 `net.inet.tcp.rack.pacing.fillcw_cap` 保存 FCC 值，以字节每秒为单位。例如，如果我想将值设置为 50Mbps，可以使用以下命令：
