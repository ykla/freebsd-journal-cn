...
在家工作已经成为新常态。但从安全的角度来看，事情变得更加复杂。安全的办公室已经不复存在，那些精心修剪过的安全边界和全天候的物理安全也已经消失。

安全专业人员谈论我们关心的风险类别，称之为我们的“威胁格局”或“安全姿态”。

这是一种花哨的说法，即我们可以做出决定，比如不关心 GCSB，KGB，CIA，摩萨德和其他政府资助的攻击者，但我们确实关心在火车上忘记笔记本电脑，或者有人的办公室被闯入，窃取高价值物品。我们关心密码和凭证。很多。

## 但是要把秘密存放在哪里呢？

特别值得注意的是，攻击者和我们作为系统管理员或开发人员的共同兴趣是管理和轮换秘密。

你见过多少环境，在那里数据库凭据从未改变过呢？长时间运行的批处理作业跨多个系统使用相同的密码？或者改变其中之一可能导致意外停机，如连锁反应般在公司内蔓延，并最终影响你的职业？

显然，便笺和凭据存放在 git 仓库中，既不具备可扩展性，也不安全。我们需要一些相对通用的解决方案，同时支持广泛的用例。

那么一个明智的 DevOps 工程师该怎么做呢？

一个解决方案是典型的密码存储，例如 BitWarden 或 1Password，并将这些工具扩展到团队中的各种命令行工具中。虽然这些工具解决了问题的一部分，但并没有解决足够多的问题。

它们主要是面向用户的工具，并不容易与复杂的 git 仓库、puppet 和 ansible 部署管道连接起来，确保凭据定期轮换，并且仅对适当的用户和系统可访问。

## 秘密管理平台

有时被称为密钥管理系统，这些通常集成到云供应商中。Azure、Amazon、Google、Oracle 都为其平台提供了紧密耦合且集成良好的工具。

但是，如果你在阅读 FreeBSD 杂志，你更有可能对把所有秘密交给公司的想法感到极度不安。理想情况下，我们希望管理自己的秘密，而不依赖于外国第三方。

## 折衷选择

这些系统专注于以高度安全和受控的方式分发和管理关键材料，使运维团队能够进行操作。
...
或者它可以提供一次性令牌，允许新配置的服务连接到指定数据库，确保令牌既未在传输过程中被劫持，也不能被多个实例使用。

当新实例被配置或部署时，它将获得一个新的一次性令牌，唯一于此实例和此时。
...
本文致力于介绍 Hashicorp Vault，其有一个功能平价的开源分支叫做 OpenBao，类似于 Terraform / OpenTofu 的许可分支。

有其他工具，但 Vault 已经移植到 FreeBSD，我相信 OpenBao 也很快就会登陆。

## Vault 的甜点

Vault 和其他 KMS 不是存储 Webstorm IDE 许可证或护照扫描副本的好地方。它不友好于最终用户，绝对不能在移动设备上使用。

但如果你正在管理服务器、数据库和网络，它非常棒。它可以轻松与 Terraform、Chef、Puppet、Ansible 以及几乎任何具有或使用命令行或终端界面的东西集成。

## 内部结构

Vault 将所有密钥和值加密存储在磁盘上。因此，在启动时需要一个主密钥来解锁所有其他密钥。为了避免单一轻量级密钥，Vault 使用 SSS 或 Shamir 的秘密分享将一个大复杂密钥分割成单独的秘密，这些秘密可以重新组合以解锁保险库。在支持 WASM 的现代 Web 浏览器中尝试 Shamir 演示。

巧妙地，SSS 允许可配置的冗余度 — 比如，需要 5 把钥匙中的 3 把来解锁保险柜。因此，您的 3 位主系统管理员可以解锁它，但如果没有任何一位在场，您可以请求您的律师或会计借用他们的钥匙（如果需要的话），并达到您的 3 位法定人数。每位管理员在本地提交其解锁钥匙，并使用 API 挑战来防止任何一位管理员获取主密钥的所有部分。
