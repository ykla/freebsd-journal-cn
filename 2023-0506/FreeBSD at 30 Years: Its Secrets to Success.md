...
多年以来，ports 一直在不断发展。最近的创新是增加了 pkg 系统来操作 ports。pkg 系统处理软件包的注册、添加、删除和升级。另一个关键组件是 Poudriere，它是一个用于创建和测试 FreeBSD 软件包的工具。它使用 FreeBSD jail 来建立隔离的编译环境。这些 jails 可用于为非实体 FreeBSD 系统的其他版本[^1]编译软件包，也可用于为不同于主机系统的体系结构编译软件包[^2]。软件包编译完成后，它们的布局就会与官方镜像完全相同。pkg 系统和其他软件包管理工具可以使用这些软件包。
