...
多年以来，ports 一直在不断发展。最近的创新是增加了 pkg 系统来操作 ports。pkg 系统处理软件包的注册、添加、删除和升级。另一个关键组件是 Poudriere，它是一个用于创建和测试 FreeBSD 软件包的工具。它使用 FreeBSD jail 来建立隔离的编译环境。这些 jails 可用于为非实体 FreeBSD 系统的其他版本[^1]编译软件包，也可用于为不同于主机系统的体系结构编译软件包（译者注：这里说的是交叉编译。）。软件包编译完成后，它们的布局就会与官方镜像完全相同。pkg 系统和其他软件包管理工具可以使用这些软件包。
...
Port、文档和开发的提交者对项目的运行都有同等的发言权。值得注意的是，他们都可以竞选核心小组，并获得相同的投票权。在大多数项目中，开发人员拥有更多的发言权，而其他人则被视为低人一等。FreeBSD 项目从一开始就致力于建立包容文化。这种文化最看重的是 “与他人相处融洽”。它不会为了得到自己的文档、port 或代码而容忍“智者”【译者注：见智者学派】的存在（尽管有时“智者”的离开或被踢出局可能需要一段时间）。
