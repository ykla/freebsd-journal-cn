...
让 FreeBSD 1.0 在真实的硬件上运行，对我来说是一个探究计算机历史的机会，尤其是在我出生之前的时代。我最早使用计算机的记忆是在 Windows 98 上——我相信在我生活中还有其他的计算机。我和朋友们玩《城堡沃尔芬斯坦》的那台机器可能是旧电脑或者过剩的办公机器。直到我有了第一台真正属于我的电脑（2000 年代中期的 G4 iBook），我开始对计算机内部和操作系统进行探索。

我迫不及待地想去探讨本文的主题。我相信编辑部可能期望本文会介绍如何在众多 x86 模拟器中运行 FreeBSD 1.0，但我不会错过任何过度做事的机会。

复古计算机正在成为一种越来越受欢迎的兴趣。在英国，慈善商店（类似于 Goodwill）很长时间内无法收受电子产品。这导致那些不愿意在寒冷的星期天早晨去车载市集上卖掉这些电脑的人选择把它们扔掉。
...
在过去的十年里，复古计算机的爱好者群体不断壮大。有像 Action Retro、Lazy Game Reviews 和 This Does Not Compute 这样的频道，它们极力将老机器推向极限。这可以通过安装从那些机器仍然得到支持的加速器卡以及通过适配器、现代存储设备和外设来实现，让机器以原本的所有者都无法想象的速度运行。
...
有一张收藏的 FreeBSD 1.0 CD 可以从该项目和互联网档案馆获得。它包含了发布软盘、源代码以及针对 FreeBSD 的 ports 和软件包。在 CD 中还包括了各种安装方法的发布公告和安装说明。
...
但是这个设备对我一点用都没有。失望而又不耐烦，我决定放弃它，彻底擦除了 MS-DOS 磁盘（谁需要 DOS，我要安装 UNIX 系统！）。我设置 FreeBSD 占据了整个 200MB 的硬盘空间。
