...
幸运的是，互联网上有本 [ZFS 磁盘布局规范](https://people.freebsd.org/~markj/Zfs_ondiskformat.pdf)；它不大完整且过时，但总比什么都没有要好。除此之外，我还可以查看引导加载程序的代码。从某种意义上说，引导加载程序解决了与 `makefs` 相反的问题：它只读取 ZFS 池中的数据，而不写入任何内容，而 `makefs` 创建了一个新的池，但不用读取现有的池。
