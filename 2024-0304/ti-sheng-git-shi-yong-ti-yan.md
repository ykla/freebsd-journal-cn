...
有时候被迫切换版本控制系统，因为供应商破产了，或者有比当前解决方案更吸引人的东西出现。最好的往往带有恶意。Git 如今已经成为版本控制的事实标准，即使它有些粗糙和一点学习曲线。在我看来，作为 FreeBSD 项目的用户/消费者以及一个 committer，重新熟悉 git 并让 git 成为为我工作（而不非其他）的工具，让我花了一些时间。我可能仍然不能完全掌握所有的内部工作方式，但幸运的是，我不必这样做。为 src 和ports 树工作的开发人员还会使用更多像分支、合并、标签、挑选、重新基于以及其他一些在文档存储库中不需要的功能。尽管如此，有时候在 ports 描述或者手册页中改正拼写错误时，还是需要像我这样，对这些概念至少有基本了解的人来做正确的事情。

问题也在于某件事情的使用频率。如果我一年内某个功能只用了一两次，就可能需要再次查找，因为上一次使用已经有一段时间了。克隆新树、拉取更新、进行更改、提交和推送这些基础操作，一旦你经常进行，就显得非常熟悉。我把它比作基本的 vi 使用。我可以打开文档，进行修改，保存并退出。但像 vi 这样强大的编辑器和其他类似工具提供的其他功能，可能会永远隐藏在我看不见的地方。同样对于 git，它在幕后有更多功能，用户并不了解或者根本不需要。然而，往往有一点点知识和配置关于这些高级功能，即使对开发者来说，也是很有帮助的，也对用户如此。
