...
版本控制历史悠久。拥有一种检索某个特定版本文件的方法，是我们旅程的开端。毕竟，这是备份软件的功能。有些人将版本控制系统（VCS）用于该用途，但这仅是皮毛。VCS 真正有价值的地方，是能够审查文件在历史上的变更。与其他人一起使用它，能使重要项目文件与其他人保持同步，在这种情况下，类似责备、差异、分支和合并的功能变得更加相关。这些功能使得在文本、源代码、配置文件等值得在群体内共享的东西上进行协作成为可能。很多将一大批此类文件纳入版本控制的大型项目，如果没有基于它构建的复杂工具，将无法维护。版本控制系统来了又去，无论是商业供应商还是开源提供商。其中大都实现了用户所确立的基本功能集，而且引入一套全新术语和复杂工具往往会被视为推广障碍。

在 FreeBSD 项目漫长的历史中，每一次的代码更改都附有一个说明文本，称为提交信息。对某些人来说，可能看起来很麻烦。但事实上，在溯源过去的问题时，很有帮助。为何在 15 年前做出了这项改变？是谁做出了这个修改，当次提交还包含了别的哪些文件，以及实际受影响的代码行是哪些？这些不仅是版本控制系统生成的差异相关联的问题，而且提交信息对于掌握如今代码中正进行着的事情是一种宝贵的帮助。这通常会贯通过去与未来，因为 Unix 系统已经存在了很久，并且曾在如今无法想象的硬件（过去做出更改的人所用）上运行着。

由于 FreeBSD 存在了相当长时间，保留这段丰富的历史极为重要。尤其是必须转换到其他版本控制系统时。随着时间的推移，FreeBSD 曾经使用过 CVS，然后迁移到了 Subversion，现在则使用着 git（我可以肯定，在早期也用过 RCS）。每当进行这样的迁移，一个主要的目标就是保留每一次的变更，以及提交信息。

有时候被迫切换版本控制系统，因为供应商破产了，或者有比当前解决方案更吸引人的东西出现。最好的往往带有恶意。Git 如今已经成为版本控制的事实标准，即使它有些粗糙和一点学习曲线。在我看来，作为 FreeBSD 项目的用户/消费者以及一个 committer，重新熟悉 git 并让 git 成为为我工作（而不非其他）的工具，让我花了一些时间。我可能仍然不能完全掌握所有的内部工作方式，但幸运的是，我不必这样做。为 src 和ports 树工作的开发人员还会使用更多像分支、合并、标签、挑选、重新基于以及其他一些在文档存储库中不需要的功能。尽管如此，有时候在 ports 描述或者手册页中改正拼写错误时，还是需要像我这样，对这些概念至少有基本了解的人才能做事。

问题也在于某物的使用频率。如果我在一年内某功能只用过一两次，就可能需要再次查阅，因为距上一次使用已经有一段时间了。克隆新树、拉取更新、进行更改、提交和推送这些基础操作，如果你经常进行，就显得非常熟悉。我把它比作基本的 vi 使用。我可以打开文档，进行修改，保存并退出。但像 vi 这样强大的编辑器和其他类似工具提供的其他功能，可能会永远隐藏在我看不见的地方。同样，对于 git，它有更多功能隐藏于幕后，用户并不了解或者压根不需要。然而，一般对于这些高级功能有点知识和配置，即使对开发者，也极有帮助的，对用户亦如此。

FreeBSD 14 弃用了工具 portsnap，许多人用它来获取和更新 ports。手册和其他地方的说明现在要求用户直接从 git 检出 ports。当用户需要 src 时，亦如此。比如他们可能需要重新编译 VirtualBox 的内核模块。这些都是很好的使用案例，但有一个问题：由于上述悠久且丰富的历史，src 和 ports 克隆需要很长时间。让我们看看，是否可以通过一些配置和工具来加速该过程。我们还将了解 git 提供的其他精彩功能，无论对普通用户张三还是开发者李四来说，都一样。

## 更快的克隆

首先，在 FreeBSD 上安装 git ，请运行 `pkg install git`。由于我们还没有 ports，况且现在没有 portsnap 了。这可能会在无法访问 FreeBSD 镜像站的系统上，形成先有鸡，还是先有蛋的悖论。在您的本地网络中，poudriere 机器可能是解决方案，或者在不同的机器上下载文件，复制二进制文件，然后用 `pkg install ./<软件包名>` 进行安装。
...
随着时间的推移，我的配置可能会不断增加，因为我发现了其他有用的选项。 Git 在配置上很灵活。默认设置对大多数人来说都很好，而且更改也很容易进行。这篇文章应该可以帮助您开始编写自己的配置，可在使用 git 时减少某些磨合问题。
