...
它简洁地描述了一个极其复杂的情绪事件。我们可以说“士兵们慌乱了”，于是便能想象一场战斗的局势。我们可以用它来表达小小的疏忽带来的分量，也可以解释当我们踏上飞机却怀疑自己是否关掉了烤箱时的脑中活动。

肯定是关了吧。

“恐慌”或许是我最喜欢的书名词汇之一：《恐慌！Unix 系统崩溃转储分析》——哇！我一定得有一本。任何作者使用这样的标题肯定写了一本好书，即使它只是一本旧的 Sun OS/Solaris 技术手册。
...
我承认，我一直不理解为什么人们如此渴望核心转储——我会想，核心转储能比堆栈跟踪给我们更多什么呢？但从《恐慌！》中，我迈出了崩溃分析的第一步，体验了在 FreeBSD 上使用尖端内核调试工具的旅程。让我来展示我学到的东西。别担心，不需要等着湿透的柠檬纸巾了。

## 获取内核转储

我得坦白，知道你不会尝试进行内核转储分析，除非真的需要。当你遇到一台卡住的机器时，这种情况下的生活就是 printf 调试的世界。

获取可用的核心转储并不难——你的系统需要设置成接收崩溃转储（参见 dumpon(8)），然后进入调试器。通常，系统会主动帮助你进入调试器，让系统陷入恐慌。FreeBSD 提供了一项功能，即使在没有出现问题时也可以让内核陷入恐慌。在测试系统上将 `debug.kdb.panic` 的 sysctl 设置为 1，即可进入调试提示符：
...
设置 `debug.kdb.panic` 后，你将进入 ddb(4) 提示符。ddb 是一个功能齐全的实时系统调试器——它是一个很好的分析工具，但今天我们不需要它。
...
当虚拟机重启时，`savecore` 会显示一条消息，提示已经提取并保存了核心文件。

核心文件将被放置在 `/var/crash` 目录中，还有其他一些文件。
...
最后，`vmcore.last` 是一个链接，指向最近的核心转储文件，以防你经历了有趣的一周并忘记了最新的崩溃记录。

### 符号

分析核心转储的第二个必要内容是内核符号。版本发布的内核符号可以通过 `kernel-dbg` 包获取，并安装到 `/usr/lib/debug/` 目录，或者可以从你的内核构建目录中提取出来。

### 使用 gdb 查看核心转储（初步）

首先，我们用 `kgdb` 简单快速地查看核心转储，作为后续使用 lldb 进行崩溃转储调试的参考点。
...
`kgdb` 启动时会显示许可证（已省略），然后打印内核消息缓冲区的最后部分，这是 `kgdb` 提供的一个很有用的功能。消息缓冲区的最后部分向我们提供了恐慌信息、一些其他信息以及堆栈跟踪。
...
在这里，我列出了导致恐慌的回溯路径，识别了大约在帧 #11 处调用的 `panic`，并要求 `kgdb` 跳转到帧 **#12**（导致恐慌的具体代码），然后列出该处的代码。进一步调查可以帮助我们找出在此崩溃转储中导致恐慌的原因。

这是内核调试的基本步骤，查看当时的情况并分析崩溃转储，以找出变量所持有的值。为了在内核上下文中有用，lldb 也需要能够完成这些任务。

### lldb

在过去的十年里，FreeBSD 一直在向更自由许可的 llvm/clang 工具链迁移。尽管调试支持一度缺失，但在 2024 年，FreeBSD 内核的调试功能终于能够在 lldb 中实现。
...
实际上，lldb 并非专为命令行界面设计，而是为了通过 API 被软件驱动。这导致了它的许多命令显得冗长。幸运的是，lldb 的命令行现在对 gdb 风格的命令支持得更多了，许多命令界面更为一致。基本命令如打印操作现已兼容，但其他选项仍然有差异，有些更好，有些则差很多。
...
在内核中有一个全局可见的 `msgbuf` 结构体，用于实现内核的消息缓冲区。lldb 可以显示缓冲区的起始部分。字段 `msg_wseq` 和 `msg_rseq` 告诉我们写入和读取的位置。
...
lldb 还提供了一个脚本接口进行控制，这也是为什么许多命令的输入非常冗长。目前，lldb 支持 C++ 和 Python 脚本，并已试验性支持 Lua。FreeBSD 的基础系统中自带 Lua，而 2024 年的 FreeBSD 版本 lldb 默认支持 Lua。
...
lldb 的 Lua 接口是从 SWIG 绑定生成的，这是一种描述库之间接口的 C++ 格式。Python 和 Lua 绑定是用相同的方式生成的。如果对 API 或如何使用有疑问，可以通过查看 lldb 项目提供的 Python API 文档找到答案。虽然这种方法不太方便，但还是可以实现。
...
lldb 的 Lua 环境提供了一个 `lldb` 变量，包含了访问目标、调试器、帧、进程和线程的成员。这些对象映射到 Python API 中描述的对象。

我并不是很喜欢 lldb 的 API，编写起来比较繁琐，如果选择的函数或变量布局有问题，理解起来也比较困难。

不过一旦积累了一些经验，就能更好地理解它的需求。

让我通过一个示例来说明如何使用 lldb 的 Lua 绑定从崩溃转储中打印消息缓冲区。

通过 lldb 的 Lua 变量，我们可以访问转储镜像中的文件。当我开始分析核心转储时，一个主要的困难是理解如何在内存中定位信息。可以从各种内核全局变量入手，大多数子系统都有可以用于构建的内容。
...
lldb 及其内置的 Lua 的一个重要价值在于它随 FreeBSD 发行版一同提供。lldb Lua 拥有自由许可协议，与 FreeBSD 兼容，从 2024 年初开始，CURRENT 构建版本中默认启用了它。这让内核开发人员和问题排查人员可以使用 lldb Lua 编写脚本，并提供给用户进行分析。
