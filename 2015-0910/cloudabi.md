...
让我们通过创建一个微型 Web 服务器来实地看看 cloudabi-run 如何运作。该 Web 服务器的配置文件采用极简格式：一组键值对列表。目前只支持名为 `socket` 的单一配置项，应持有一个指向网络套接字的文件描述符。最终目标是支持更多配置选项，例如选项 `rootdir` 用于提供指向 Web 服务器根目录的文件描述符。让它运转起来就留作练习吧。
