...
revision 272274 中的变更对系统管理员和开发者都很重要，尤其是使用 root-on-ZFS 安装时——默认的 **/var** 数据集现在设置了 `canmount=off` ZFS 属性，因此虽然 **/var** 默认会创建，但不会在启动时自动挂载。这让系统可以拥有多个 **/var** 数据集，配合 Port sysutils/beadm 等多引导环境使用时，可防止多个独立环境的 **/var** 目录相互冲突。
