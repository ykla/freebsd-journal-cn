...
FreeBSD 的声音支持始于 1993 年，当时 Jordan K. Hubbard 将通用 Linux 声卡驱动移植到 FreeBSD，该驱动后来被称为 VoxWare 声卡驱动，由 Hannu Savolainen 编写。1993 年至 1997 年间，多个新版本的 VoxWare 驱动被导入（并修改）到 FreeBSD 基础系统。当时，Amancio Hasty 和 Jim Lowe 负责 FreeBSD 声音支持的大部分工作。VoxWare 驱动最终发展成为我们今天所知的 OSS（Open Sound System，开放声音系统），由 Hannu 及其团队在 4Front Technologies 维护。

然而，1997 年情况发生变化，当时 Luigi Rizzo 构建了现代 FreeBSD 声音系统的基础。1999 年，Cameron Grant 为 FreeBSD 4.0 重写了声卡系统，使用了 newbus 接口，从而支持大量硬件。这取代了 Luigi 的驱动，其驱动在一个月后被从代码树中移除。随后几年，声音领域经历了诸多变化。针对不同硬件芯片的驱动数量显著增加（尤其是 PCI 和 USB 设备），声卡基础设施也得到了多项改进，这都得益于 Cameron Grant 和 Orion Hodson。Cameron 于 2005 年去世，这是 FreeBSD 社区的一大损失。

2005 年，Ariff Abdullah 接管了 FreeBSD 声音代码的维护工作。自此，FreeBSD 声音支持经历了显著变化，并进行了多轮设备驱动重构。

## OSS

FreeBSD 的声音 API 被称为 OSS，即 “Open Sound System”（开放声音系统）。有趣的是，它曾经也是 Linux 的默认声音 API，后来被 ALSA 取代。而 FreeBSD 仍在使用 OSS，它通过标准设备文件提供了简单清晰的接口（`/dev/dsp*` 对应声卡，`/dev/mixer*` 对应混音器），并通过少量常用 POSIX 系统调用进行操作：
...
然而，要实现更复杂的功能，则需要编写实际的程序——幸运的是，OSS API 使用起来非常直观。`/usr/share/examples/sound` 包含多个示例程序，并且在本文撰写时仍在不断增加。

除了 `/dev/dsp*` 设备外，还有一个 `/dev/dsp` 设备，上述示例中使用的就是它。它并不是一个真实的设备，而是当前默认设备的别名。建议应用程序在需要使用当前默认设备时访问该别名，而不要硬编码特定设备。

OSS 的另一个优点是它只依赖 POSIX 系统调用，因此可以非常容易地在任何编程语言中使用，无需语言绑定。若想将 OSS 完全“移植”到其他语言，只需定义一些全局 OSS API 常量和结构体，这些定义可在 `/usr/include/sys/soundcard.h` 中找到。
...
驱动在初始化阶段必须附加到 `sound(4)` 并建立与其的通信管道。换句话说，`sound(4)` 是用户空间与设备驱动之间的桥梁。这非常方便，因为 FreeBSD 内核为每个连接的声卡都暴露相同的 `/dev/dsp*` 文件和 sysctl（`hw.snd.*` 和 `dev.pcm.*`），为用户和应用程序提供统一访问和配置，同时避免驱动层的重复实现。

`sound(4)` 还提供 `/dev/sndstat`，显示所有已连接声卡和活动通道的信息，并被 `virtual_oss(8)` 和 `sndctl(8)` 内部使用。
...
`snd_driver` 是一个元驱动，会加载所有可用驱动。一旦确定哪个驱动与你的声卡匹配，就可以只加载该驱动。
...
* 现在支持热拔插。使用旧版 FreeBSD 的 USB 声卡用户可能记得，热拔插声卡通常会导致 USB 总线挂起，直到手动终止使用已断开设备的应用程序
  ([PR 194727](https://bugs.freebsd.org/bugzilla/show_bug.cgi?id=194727))。
* 浮点音频支持。这有点容易误解，因为至少目前在设备驱动层并未真正支持浮点音频，而是允许用户空间应用程序使用 OSS 进行浮点音频处理。这已修复了不少 Ports，例如 Wine，需要 OSS 提供浮点音频支持。
...
`virtual_oss(8)` 是由已故 Hans Petter Selasky 编写的 OSS 强大音频服务器。多年来它一直作为 ports（audio/virtual_oss）提供，但自 FreeBSD 15.0 起已成为基础系统的一部分。它目前正在积极开发中，已有许多重要改进在进行和规划中。

如 [15.0 发布说明](https://cgit.freebsd.org/src/commit/?id=c457acb4ee821cf015930a94f52c3870786468a7) 所述，FreeBSD 15.0 之前的 `virtual_oss(8)` 用户可以卸载 audio/virtual_oss port 并使用基础系统版本。唯一需要注意的是，某些依赖第三方库的功能已移至独立 port，包括：
...
`mididump(1)` 是一个简单工具，用于实时打印指定设备的 MIDI 事件。这对于确认 MIDI 设备工作正常，以及按键是否正确映射，非常有用。
...
你可能会觉得这是个玩笑，但实际上，近年来这个话题越来越多地被讨论，我们也已经在一些会议上看到相关演讲，例如：
...
我真诚地认为，如果我们继续保持和开发声音子系统，移植和开发更多软件，并在实践中展示 FreeBSD 在音频和音乐制作方面的潜力，无论是在会议上还是在线，我们有朝一日可能会看到 FreeBSD 在发烧友和音乐人群体中获得显著关注和认可。

## 报告与解决 bug

所有软件都有可能出现 bug，声音子系统也不例外。提供足够的信息非常必要，仅仅提交“我的机器上声音无法工作”这样的描述，并不能真正帮助定位问题。通常附上以下命令的输出即可满足大部分情况的需求：
...
希望本文能够帮助读者了解当前 FreeBSD 声音子系统的整体结构。未来，如果能有更多人关注 FreeBSD 的声音系统，那将是非常好的事情！大部分讨论集中在 [freebsd-multimedia@FreeBSD.org](mailto:freebsd-multimedia@FreeBSD.org) 邮件列表，因此建议持续关注该列表。

---

Christos Margiolis 是来自希腊的独立承包商，同时也是 FreeBSD 源代码提交者（src committer）。
