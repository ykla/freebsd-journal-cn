...
Mali GPU 有多个线程用于不同的操作。例如，Midgard GPU 使用复杂的 tiling 架构。它有三个线程负责顶点着色器；一个 tiler 负责将三角形排序到 tile 中并传递给片段着色器；片段着色器则处理传入 tile 上的最终光栅化（即写入帧缓冲）。

Mali GPU 支持 Vulkan 1.2、OpenGL ES 3.2、OpenCL 1.2（Midgard 架构）、OpenCL 2.0（Bifrost 架构）以及 OpenCL 2.1（Valhall 架构）。Mali GPU 还支持 SPIR-V。
...
由于 Panfrost 驱动程序支持各种 GPU 型号和版本，它还负责为特定模型的硬件问题设置怪癖和变通方法。
...
LKPI 使得将 Linux DRM 驱动程序带到 FreeBSD 变得更容易，修改最小，并能够保持与 Linux 上游的同步。2018 年晚些时候，drm1 和 drm2 被从内核移到 Ports 树，作为 Port drm-legacy-kmod；而 Port drm-next 则采用了原始名称"drm-kmod"。
...
在基于 FDT 的平台（如 arm 和 arm64）上，DRM 的情况稍有不同：我们希望将 DRM 保持在内核中，因为它更适合嵌入式环境，并且我们不想从 Linux 导入 arm 图形驱动程序。一个原因是，基于 FDT 的资源（gpio/pinmux/clock/regulator）框架在 Linux 和 FreeBSD 之间不同，这可能使得 LinuxKPI 抽象变得非常复杂。此外，驱动程序需要与我们定期从 Linux 导入的 DTS 文件同步。
...
在 <https://github.com/evadot/drm-subtree> 上，你可以找到一组可工作的 DRM/KMS 驱动程序和适用于 Allwinner、Nvidia Tegra 和 Rockchip SoC 的说明，这些驱动程序允许在 FreeBSD 下启用显示功能。
