...
FreeBSD 11 包含对附加工具链软件包和 `CROSS_TOOLCHAIN` 的支持。在 FreeBSD 12 开发周期中，工作集中于进一步细化此支持。例如，通过补丁和对工具链软件包的配置更改，改进了对标志 `--sysroot` 的支持。此外，构建系统更新为对外部工具链更友好，例如尽可能使用编译器驱动程序链接二进制文件，并支持不同的 MIPS ABI（如 N32）。
