...
Fabrics 主机和控制器通过 NVMe 合格名称（NQN）进行标识。NQN 是 ASCII 字符串，应以“nqn.YYYY-MM.reverse-domain”开头，后跟可选的后缀。名称的反向域部分应该是一个有效的 DNS 名称的反向顺序，YYYY 和 MM 字段应该指定为，由使用该前缀的组织，所有的 DNS 域名的有效年份和月份。规范定义了一个固定的子系统 NQN 用于发现控制器，并且定义了从 UUID 构建 NQN 的方案。在建立关联时，必须指定主机和子系统（控制器）的 NQN。
