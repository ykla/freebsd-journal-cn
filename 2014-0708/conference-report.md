...
FreeBSD 文档冲刺（Doc Sprint）有两大亮点：一是 mandoc 项目的 Ingo Schwarze 参与，他将 FreeBSD 的 Igor 文档校对工具提交到了 OpenBSD Ports；二是 Allan Jude 凭借文档工作正式进入 FreeBSD 项目并获得提交权限。Allan 和 Kris Moore 通过 BSDNow 播客提升了 FreeBSD 和其他 BSD 项目的知名度，并展示了社区参与和代码提交可以多么无缝衔接。
...
DevSummit 的其他亮点包括澄清 FreeBSD 的“长期支持”政策，令人欣慰的是项目实际上或多或少在遵循提议的 5 年政策。对这类政策的正式确认对从供应商到终端用户的每个人都是宝贵的营销工具。还有人提出将 FreeBSD 基本系统拆分为软件包，以便模块化更新和部署。处理得当的话，这对嵌入式 FreeBSD 工作将大有裨益。
