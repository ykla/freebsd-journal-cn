...
简短的回答是“不是，仍然是那个熟悉的加载程序。”但我们正在努力使它更友好并支持更多的功能。
...
最显著的变化是启动加载程序的图形控制台。虽然当前的实现还不完美，并且可以改进，但我希望大多数用户会喜欢更新后的外观。

加载程序的控制台终端模拟器 teken 取自内核树。在此基础上，我们可以设置前两个调优参数：
...
目前，我们提供了 Terminus 家族的控制台字体，安装在目录 **/boot/fonts** 中。加载程序内置了 8x16 字体，但为了节省空间，内置字体仅提供 ASCII 集。
...
首先，环境变量 `screen.font` 可用，并允许更改所用字体。若使用无效值或取消该值，会在控制台上打印当前可用字体的列表。

其次，命令 `loadfont` 允许用户加载自定义字体文件，例如 **/boot/fonts/gallant.fnt**。字体需要用 vtfontcvt(8) 工具准备。

加载程序中的字体索引假定字体大小唯一。当已注册 8x16 字体文件时，尝试加载提供相同字体大小的不同字体文件会用新文件替换之前加载的文件。
...
为了构建更美观的画面，启动加载程序支持显示 PNG 格式的图像文件。目前，我们要求图像为带有 alpha 通道的 TrueColor，并且支持图像缩放。可以在 `drawer.lua` 和 `gfx-orb.lua` 文件中找到如何在启动加载程序菜单中使用图像作为徽标或品牌组件的示例。

## 缺点

某些系统在使用帧缓冲控制台时会遇到控制台变慢的情况。使用 VBE 帧缓冲时，一种可能的解决方法是使用较小的颜色深度——默认是 32 位颜色。对于 VBE 和 UEFI，或许可以使用较小的分辨率，并在 KMS 驱动程序运行后配置更好的分辨率。当然，在某些情况下，唯一合理的选项可能是使用文本控制台。
