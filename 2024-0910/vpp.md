...
要为我们的网络创建三个节点，我们将利用 FreeBSD 最强大的功能之一——VNET jail。VNET jail 为我们提供了完全隔离的网络堆栈实例，它们的操作方式类似于 Linux 的网络命名空间（Network Namespaces）。要创建 VNET，我们需要在创建 jail 时添加选项 `vnet`，并传递它将使用的接口。
...
在我们的示例中，我们将使用 VPP 与 Netmap，Netmap 是高性能的用户空间网络框架，作为 FreeBSD 的默认组件提供。使用 Netmap 之前，接口需要一些配置——接口需要处于启用状态，并且需要配置选项 `promisc`。
