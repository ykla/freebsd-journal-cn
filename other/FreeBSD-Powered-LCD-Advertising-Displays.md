...
为简化工作，在每个 USB 存储设备或 SATA SSD 上都创建了一个 MBR 分区表。每个存储设备只创建一个分区，并在该分区中创建一个 BSD label “a”。切片 `/dev/da0s1a`（用于 USB 存储设备）或 `/dev/ada0s1a`（用于 SATA SSD）被格式化为启用软更新和日志 的 FreeBSD FFS 2。
...
**表 3. USB 存储设备或 SATA SSD 中文件系统的概览布局。**
